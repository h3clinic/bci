#!/usr/bin/env python3
"""
gen_schematic_v4.py — Hardened KiCad 9 schematic generator
==========================================================

Structural guarantees over v3:
  1. pin_abs() is the ONLY coordinate transform — no inline math
  2. Lane assignment per channel — each channel owns a unique X column
  3. Endpoint uniqueness assertion — collision = build failure
  4. Deterministic routing — horizontal to lane, then vertical
  5. Labels placed exactly on computed wire endpoints
  6. Dangling globals get no_connect markers or stub+label

Architecture: invasive in-vitro BCI headstage
...
class WireRegistry:
    ...
    def register(self, x, y, net_name):
        """Register a wire endpoint for a given net.
        Returns (x, y) rounded to grid.
        """
    ...
    def register_junction(self, x, y):
        """Junctions are intentional multi-net meeting points."""
        key = (round(x, 2), round(y, 2))
        # Don't overwrite — junctions just mark "this overlap is OK"
        return key

    def check(self):
        """Call at the end. Raises if any violations found."""
        if self._violations:
...
# ═══════════════════════════════════════════════════════════════════════════
# 3. SYMBOL DEFINITIONS  (unchanged from v3 — just the s-expression text)
# ═══════════════════════════════════════════════════════════════════════════
...
class Sch:
    ...
    def wire(self, x1, y1, x2, y2, net="__wire__"):
        """Emit a wire.  Both endpoints are registered in the collision checker."""
    ...
    def label(self, name, x, y, rot=0):
        """Net label — MUST be placed exactly on a wire endpoint."""
        self.items.append(f"""
  ...
  (paper "A3")
  (title_block
    (title "Neural AFE Headstage — In Vitro BCI")
    (date "2025-02-23")
    (rev "0.4")
    ...
    (comment 3 "B2B: Samtec QSH-030 | fs=30kS/s | 16-bit | invasive in-vitro")
  )
  (lib_symbols
{lib_syms}
  )
...
# ═══════════════════════════════════════════════════════════════════════════
#
# Each electrode channel gets a dedicated X lane for all vertical routing.
# No two channels can share the same X column.
#
# Layout:
#   RHD input pins are on the LEFT side of the chip.
#   Bias resistors are placed LEFT of the chip, each in its own lane.
#   Route: RHD_pin → horizontal left → lane_x → vertical to resistor
#
#            lane0  lane1  lane2  lane3          RHD
#              |      |      |      |             |
#   R1(CH0)---+      |      |      |             | in0
#              |  R2(CH1)---+      |             | in1
#              |      |  R3(CH2)--+             | in2
...
LANE_PITCH = g(6)  # 15.24mm between channel lanes — generous, no overlap possible
...
def build():
    ...
    # ┌──────────────────────────────────────────────────────────────────┐
    # │  ELECTRODE INPUTS — LANE-BASED ROUTING                           │
    # └──────────────────────────────────────────────────────────────────┘
    #
    # Strategy:
    #   1. Compute lane_x for each channel: BIAS_BASE_X + i * LANE_PITCH
    #   2. Place bias resistor R(i) centered at (lane_x, R_CENTER_Y)
    #   3. Route: RHD_pin(in_i) → horizontal LEFT to lane_x → vertical DOWN to R top
    #   4. R bottom → GND (short vertical stub)
    #   5. Label "CHi" placed on the horizontal wire segment
    #
    # The RHD in0..in3 pins are at the LEFT side of the chip, spaced g(1) apart vertically.
    # We route them LEFT, each to its own lane, then DOWN to the resistor.

    # Compute the leftmost RHD input pin X (all left pins share the same X)
    in0_x, in0_y = rhd_pin(RHD_X, RHD_Y, "in0")

    # Bias resistor block: placed well LEFT of the RHD, with room for lanes
    BIAS_BASE_X = in0_x - g(20)  # ~50mm left of RHD input pins
    R_CENTER_Y_OFFSET = g(6)     # resistor center is 6G below the horizontal routing rail
    ...
    for i in range(NUM_CHANNELS):
        ...
        # Lane X for this channel — guaranteed unique
        ...
        r_top = passive_pin1(R_X, R_Y)   # top connection
        r_bot = passive_pin2(R_X, R_Y)   # bottom connection

        # Route: RHD_pin → horizontal to lane_x → vertical to R top
        # Segment 1: horizontal from RHD pin to lane_x (at inp_y)
        s.wire(inp_x, inp_y, lane_x, inp_y, net=ch_net)

        # Segment 2: vertical from lane_x at inp_y down to R top
        ...
        # Label: placed on the horizontal segment, at lane_x (on the wire)
    ...
    # Signals that exist on the B2B but have no connection on THIS chip
    # (they're for the second RHD2164 or test features).
    # Instead of leaving global labels dangling, we mark them no_connect.
    ...
    for i in range(B2B_NPINS):
        ...
        if pname == "GND":
            ...
            # Mark as NC for this partial build — no dangling global labels
            s.nc(px, py)
        else:
            # Connected signals: CS1, SCLK, MOSI, MISO1_A, MISO1_B
...
def main():
    ...
    print("Generating schematic v4 (hardened)...")
    sch = build()

    # Endpoint collision check
    print(f"  Wire registry: {WREG.summary()}")
    WREG.check()  # exits with error if collision found
    ...
    proj = {
        ...
        "net_settings": {"classes": [{"bus_width": 12, "clearance": 0.2,
            "name": "Default", "track_width": 0.2, "via_diameter": 0.6,
    ...
    # sym-lib-table (absolute paths)
    with open(os.path.join(outdir, "sym-lib-table"), "w") as f:
        f.write("""(sym_lib_table
  (version 7)
  (lib (name "power")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/symbols/power.kicad_sym")(options "")(descr "Power symbols"))
  (lib (name "Device")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/symbols/Device.kicad_sym")(options "")(descr "Generic devices"))
  (lib (name "afe")(type "KiCad")(uri "${KIPRJMOD}/afe_symbols.kicad_sym")(options "")(descr "Custom AFE symbols"))
)
""")

    # fp-lib-table (with real paths)
    with open(os.path.join(outdir, "fp-lib-table"), "w") as f:
        f.write("""(fp_lib_table
  (version 7)
  (lib (name "Resistor_SMD")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/footprints/Resistor_SMD.pretty")(options "")(descr ""))
  (lib (name "Package_BGA")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/footprints/Package_BGA.pretty")(options "")(descr ""))
  (lib (name "Package_TO_SOT_SMD")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/footprints/Package_TO_SOT_SMD.pretty")(options "")(descr ""))
  (lib (name "Connector_Samtec")(type "KiCad")(uri "/Applications/KiCad/KiCad.app/Contents/SharedSupport/footprints/Connector_Samtec.pretty")(options "")(descr ""))
)
""")
